            ...
            if (TryGetTileEntityAs(i, j, out StarlightBeaconCosmeticTileEntity entity))
            {
                ...
                if (tile.TileFrameX == 0 && tile.TileFrameY == 0)
                {
                    ...
                    if (deployTimer > 0)
                    {
                        ...
                        for (int h = 0; h < 2; h++)
                            RadianceDrawing.DrawBeam(pos, new Vector2(pos.X, 0), h == 1 ? new Color(255, 255, 255, entity.beamTimer).ToVector4() * mult : new Color(0, 255, 255, entity.beamTimer).ToVector4() * mult, 0.2f, h == 1 ? 10 : 14, RadianceDrawing.SpriteBatchData.TileDrawingData);

                        RadianceDrawing.DrawSoftGlow(pos - Vector2.UnitY * 2, new Color(0, 255, 255, entity.beamTimer) * mult, 0.25f, RadianceDrawing.SpriteBatchData.TileDrawingData);
                    }
                }
            }
            return false;
