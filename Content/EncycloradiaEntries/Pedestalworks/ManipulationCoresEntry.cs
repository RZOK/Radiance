    ...
    public class ManipulationCoresEntry : EncycloradiaEntry
    {
        ...
        public override void PageAssembly()
        {
            AddToEntry(this, new TextPage()
            {
                ...
                @"Firstly, the \b Orchestration Core \r will act to transport items over distances. If an item is dropped near a pedestal that has an \b Orchestration Core \r atop it, and that pedestal is linked by a ray to output to another pedestal that also has a proper core, the dropped items will quickly be relocated to that pedestal, at the cost of a tiny amount of \y Radiance. \r | " + 
                @"\b Orchestration Cores \r only have a cooldown for sending items, not recieving. This means you can daisy-chain numerous pedestals together, all with cores atop them, to transport items longer distances than you normally could with only a single pair. | " +
                @"\b Orchestration Cores \r also come with the ability to output \y Radiance, \r meaning you can power an entire line of them without any extra unnecessary rays. | " +
                ...
                @"The ability to toggle the activity of an items atop pedestals with wiring also proves to be an advantage, as it allows you to disable it all at once with a single lever flip, as opposed to trying to scoop up the deleting matter with a metal bucket. |" + 
